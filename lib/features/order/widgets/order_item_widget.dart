...
import 'package:crystal_code/features/order/enums/order_status_enum.dart';
import 'package:crystal_code/helper/custom_snackbar_helper.dart';
import 'package:crystal_code/helper/date_converter_helper.dart';
...
import 'package:crystal_code/utill/color_resources.dart';
...
import 'package:crystal_code/features/order/widgets/re_order_dialog_widget.dart';
                        ...
                        Expanded(
                          ...
                          children: [
                            Container(
                              ...
                              child: Text(
                                getTranslated(
                                    orderList![index].orderStatus, context),
                                style: rubikRegular.copyWith(
                                    color: OrderStatus.pending.name ==
                                            orderList![index].orderStatus
  ...
  Widget build(BuildContext context) {
    return Consumer<OrderProvider>(builder: (context, orderProvider, _) {
      return CustomButtonWidget(
        isLoading:
            orderProvider.isLoading && index == orderProvider.getReOrderIndex,
        radius: Dimensions.radiusSizeFifty,
        btnTxt: getTranslated(
            isRunning || orderList![index].orderType == 'pos'
                ? 'track_order'
                : 'reorder',
            context),
        ...
        onTap: () async {
          if (isRunning || orderList![index].orderType == 'pos') {
            Navigator.pushNamed(
                context, Routes.getOrderTrackingRoute(orderList![index].id));
          } else {
            if (!orderProvider.isLoading) {
              ...
              if (cartList != null && cartList.isNotEmpty) {
                ...
                showCustomSnackBar(
                    getTranslated('product_not_available', Get.context!),
                    Get.context!);
              }
            }
          }
